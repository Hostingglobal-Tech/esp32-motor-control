...
WiFi 연결 순서 (자동):
1. `config.h` 의 `WIFI_SSID` 가 설정돼 있으면 그 공유기 먼저 (기본값은 비어 있음 — 비밀번호를 코드에 넣지 않아도 됨)
2. 공개(암호 없는) AP 스캔 → 신호 센 순으로 최대 5곳 자동 연결
3. 둘 다 실패 시 자체 AP(`ESP32-MOTOR` / `motor1234`) → 폰으로 접속 후 `http://192.168.4.1`

주의: 공개 AP 는 암호화가 없는 망이다. 이 장치는 모터 명령만 주고받으므로 실용상 문제 없지만, 민감한 용도로 확장할 땐 `config.h` 에 본인 공유기를 지정하는 편이 안전하다.
